	...
	private Document getDocument(File file){
	  ...
	  //path of the file
	    ...
	    if(file.getName().contains(".htm")||file.getName().contains(".html")){
	    	...
	    	Field FileHtmlTitle = new Field(LuceneConstants.FILE_HTML_TITLE, htmlContent.substring(htmlContent.indexOf("<TITLE>") + 7, htmlContent.indexOf("</TITLE")), fileHtmlTitle);
	    	document.add(FileHtmlTitle);
	    	contentValue = new Field(LuceneConstants.CONTENTS, StringPorterStemmer(RemoveTags(htmlContent.substring(htmlContent.indexOf("<BODY>") + 6, htmlContent.indexOf("</BODY"))))+" "+ htmlContent.substring(htmlContent.indexOf("<TITLE>") + 7, htmlContent.indexOf("</TITLE")), contentField);
	    }
	    else
	    {
	    	contentValue = new Field(LuceneConstants.CONTENTS, porterstemmer(file) , contentField);
	    }
	    Field filePathValue = new Field(LuceneConstants.FILE_PATH, file.getCanonicalPath(), filePathField);
	    Field fileTimeStamp = new Field(LuceneConstants.FILE_TIMESTAMP, sdf.format(file.lastModified()).toString() ,fileTimeStampField);
